    ...
    pub fn resolve(global: &RawConfig, project: &RawConfig) -> Result<Self> {
        ...
        let watchdog = WatchdogConfig {
            ...
            patterns: WatchdogPatterns {
                ...
                auth_failure: project
                    ...
                    .as_ref()
                    .and_then(|w| w.patterns.as_ref())
                    .and_then(|p| p.auth_failure.clone())
                    .or_else(|| {
                        global
                            .watchdog
                            .as_ref()
                ...
                task_complete: project
                    ...
                    .as_ref()
                    .and_then(|w| w.patterns.as_ref())
                    .and_then(|p| p.task_complete.clone())
                    .or_else(|| {
                        global
                            .watchdog
                            .as_ref()
                ...
                sandbox_violation: project
                    .watchdog
                    .as_ref()
                    .and_then(|w| w.patterns.as_ref())
                    .and_then(|p| p.sandbox_violation.clone())
                    .or_else(|| {
                        global
                            .watchdog
                            .as_ref()
        ...
        let sandbox_defaults = {
            ...
            SandboxPolicyConfig {
                ...
                extra_reads: policy
                    .and_then(|p| p.extra_reads.clone())
                    .unwrap_or_default(),
                extra_writes: policy
        ...
        for (name, raw) in &project.providers {
            ...
            if let Some(flag) = &raw.resume_flag {
                entry.resume_flag = Some(flag.clone());
            }
        ...
        let splinter_defaults = SplinterDefaults {
            cli_provider: project
                .splinter_defaults
                .as_ref()
                .and_then(|s| s.cli_provider.clone())
                .or_else(|| {
                    global
                        .splinter_defaults
                        .as_ref()
            ...
            fallback_cascade: project
                .splinter_defaults
                .as_ref()
                .and_then(|s| s.fallback_cascade.clone())
                .or_else(|| {
                    global
                        .splinter_defaults
                        .as_ref()
    ...
    pub fn validate(&self) -> Vec<ConfigError> {
        ...
        if self.global.max_splinters < 1 || self.global.max_splinters > 32 {
            errors.push(ConfigError {
                field: "global.max_splinters".into(),
                reason: "must be between 1 and 32".into(),
            });
        }
        if self.global.tmux_session_name.is_empty()
            || self.global.tmux_session_name.contains(char::is_whitespace)
        {
            errors.push(ConfigError {
                field: "global.tmux_session_name".into(),
                reason: "must be non-empty and contains no whitespace".into(),
            });
        }
        ...
        if self.watchdog.poll_interval_ms < 500 {
            errors.push(ConfigError {
                ...
                reason: "must be >= 500".into(),
            });
        }
        if self.watchdog.scan_lines < 1 || self.watchdog.scan_lines > 5000 {
            errors.push(ConfigError {
                field: "watchdog.scan_lines".into(),
                reason: "must be between 1 and 5000".into(),
            });
        }
        ...
        {
            errors.push(ConfigError {
                field: "recorder.failover_context_lines".into(),
                reason: "must be between 10 and 1000".into(),
            });
        }
        if self.recorder.log_rotation_max_mb < 1 {
            errors.push(ConfigError {
                field: "recorder.log_rotation_max_mb".into(),
        ...
        for (name, provider) in &self.providers {
            if provider.binary.is_empty() {
                errors.push(ConfigError {
                    field: format!("providers.{}.binary", name),
        ...
        for (name, agent) in &self.agents {
            if !self.providers.contains_key(&agent.cli_provider) {
                errors.push(ConfigError {
                    field: format!("agent.{}.cli_provider", name),
                    reason: format!("unknown provider '{}'", agent.cli_provider),
                });
            }
            for fallback in &agent.fallback_cascade {
                if !self.providers.contains_key(fallback) {
                    errors.push(ConfigError {
                        field: format!("agent.{}.fallback_cascade", name),
                        reason: format!("unknown provider '{}'", fallback),
                    });
                }
                if fallback == &agent.cli_provider {
                    errors.push(ConfigError {
                        field: format!("agent.{}.fallback_cascade", name),
                        reason: "cannot contain the primary cli_provider".into(),
                    });
                }
            }
        }
        ...
        if self.global.telegram_enabled && self.telegram.allowed_chat_ids.is_empty() {
            errors.push(ConfigError {
                field: "telegram.allowed_chat_ids".into(),
                reason: "must be non-empty when telegram_enabled is true".into(),
            });
        }
        if self.telegram.token_env.is_empty() {
            errors.push(ConfigError {
                field: "telegram.token_env".into(),
                reason: "must be non-empty".into(),
            });
        }
        ...
        if !self
            .providers
            .contains_key(&self.splinter_defaults.cli_provider)
        {
            errors.push(ConfigError {
                field: "splinter_defaults.cli_provider".into(),
                reason: format!("unknown provider '{}'", self.splinter_defaults.cli_provider),
            });
        }
        for fallback in &self.splinter_defaults.fallback_cascade {
            if !self.providers.contains_key(fallback) {
                errors.push(ConfigError {
                    field: "splinter_defaults.fallback_cascade".into(),
                    reason: format!("unknown provider '{}'", fallback),
                });
            }
        }
    ...
    let sandbox = if let Some(raw_sb) = &raw.sandbox {
        SandboxPolicyConfig {
            ...
            extra_reads: raw_sb
                .extra_reads
                .clone()
                .unwrap_or_else(|| defaults.extra_reads.clone()),
            extra_writes: raw_sb
                .extra_writes
                .clone()
...
mod tests {
    ...
    fn test_resolve_defaults() {
        ...
        assert_eq!(
            resolved.sandbox_defaults.network,
    ...
    fn test_resolve_merge_providers() {
        ...
        let resolved = EffectiveConfig::resolve(&global, &project).unwrap();
        assert_eq!(
            resolved.providers.get("claude").unwrap().binary,
            "project-claude"
        );
    ...
    fn test_validate_invalid_values() {
        ...
        assert!(errors
            .iter()
            .any(|e| e.field == "watchdog.poll_interval_ms"));
